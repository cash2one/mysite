...
class Alipay(object):
    ...
    def xmlToArray(self, xml):
        ...
        for child in root:
            ...
            if child.tag=='response':
                trade_status=child.find("trade").find("trade_status").text
                array_data['trade_status'] = trade_status
        return array_data
    ...
    def single_trade_query(self, **kw):
        ...
        print "=====url=%s==========" % url
        xml = requests.get(url, headers={'connection': 'close'}).text
        print "=====xml==========" 
        print "%s" % xml
        result = self.xmlToArray(xml)
        print "=====_result=%s==========" % result
        return result
